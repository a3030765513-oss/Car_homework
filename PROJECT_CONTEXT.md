...
- **分布式多机**：见 `分布式部署指南.md`（同一套 Redis + MQ）  
...
| `user-management.html` | 管理员审核用户 |
| `js/app.js` | WebSocket、Canvas、回放、完成保存弹窗 |
| `js/auth.js` | Token、`/api/auth/me`、权限 |
...
| **统计分析（Person D）** | 完成时可选保存 → `localStorage` 键 `sim_records`（含各车 steps/effectiveSteps）；`analysis.html` 卡片列表 + Canvas 详情图；设计见 `display/.../统计分析模块设计文档.md` |
...
| 统计分析无数据 / 图表平淡 | `sim_records`、`analysis.js`；改进方向见 `统计分析改进方案.md` |
...
6. `AnalysisEngine` / `doc/statistics-analysis-design.md` 中后端分析 API 多为**空壳**；当前统计 UI 走 **localStorage**，不依赖 SQL。  
7. Person D 分支 `wsh_test` 的 display 已部分合入 `hzx_common`（统计页 + 保存弹窗）。

### 扩展文档索引

| 文档 | 内容 |
|------|------|
| `分布式部署指南.md` | 多机分工、端口、联调 |
| `启动健壮性方案.md` | 乱序启动、就绪闸门（设计稿，未实现） |
| `统计分析改进方案.md` | 统计图表改进建议（P0～P3） |
| `display/.../统计分析模块设计文档.md` | 已实现统计模块数据流 |
| `CLAUDE.md` | 编码规范、编译与测试闭环 |

---

*文档版本：2026-06-22（hzx_common @ 8bb3648 起补充）*
