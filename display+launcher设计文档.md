...
L6: 热力图（选做）   ← 数据源: mapHeat[r][c]（需额外读取 Redis）
    └── 半透明色调覆层：1次=深蓝 → 2次=绿 → 3次=黄 → 4次=橙 → 5次+=红
        Person B 的 Car 模块已在 MoveExecutor 中调用 incrementMapHeat()，
        Redis mapHeat hash 已有数据积累，前端预留渲染接口即可。
```
